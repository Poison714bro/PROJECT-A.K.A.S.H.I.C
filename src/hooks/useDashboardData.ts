    ...
    async function load() {
      ...
      try {
        ...
        if (alertRes.ok && alertRes.data) {
          ...
          const dynamicAlerts = [
            { id: "a1", severity: "critical" as const, title: "Cartel Activity", description: "Suspected Sinaloa cartel movement.", payload: "", timestamp: new Date().toISOString(), source: "Signal", acknowledged: false },
            { id: "a2", severity: "high" as const, title: "Tumbler detected", description: "Tornado Cash deposit detected.", payload: "", timestamp: new Date().toISOString(), source: "Blockchain", acknowledged: false },
          ];
